...
[player, format ["%1 (%2) Teleported %3 (%4) from position: %7 (Gridref: %8) - To position: %5 (Gridref: %6).",profileName,getPlayerUID player,_target getVariable ["realname",name _target],getPlayerUID _target,getPos player,mapGridPosition player,getPos player,getPos _target,mapGridPosition _target], "https://rcon.phoenixrp.co.uk/api/v1/log/admin"] remoteExecCall (["fnc_sendHook", "PHXSRV"] call SOCK_fnc_getRemoteDestination);
