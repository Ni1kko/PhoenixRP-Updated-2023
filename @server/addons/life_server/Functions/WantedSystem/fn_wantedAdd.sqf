...
private _index = [_type,_crimesConfig] call LIFE_fnc_index;
...
if (_ret) then {
    [format["%1 has been added to %2's record.",((_crimesConfig select _index) select 0), _unit getVariable ["realname",name _unit]]] remoteExec ["PHX_fnc_notify", west];
};