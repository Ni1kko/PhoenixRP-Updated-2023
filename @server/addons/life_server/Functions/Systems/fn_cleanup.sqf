...
for "_i" from 0 to 1 step 0 do {
    ...
    uiSleep (60 * 60);
    {
        ...
        if ((!isNil {_veh getVariable "NPC"} && (_veh getVariable "NPC")) || (typeOf(_veh) in (LIFE_SETTINGS(getArray,"drones")))) then {_protect = true;};

        if ((_vehicleClass in ["Car","Helicopter","Ship","Armored","Submarine"]) && {!(_protect)}) then {
            ...
            if (isNull _veh) then {
                if (count _dbInfo > 0) then {
                    ...
                    _query = format ["UPDATE phxcars SET active='0', fuel='%2' WHERE id='%1'", [_id] call DB_fnc_numberSafe,_fuel];
