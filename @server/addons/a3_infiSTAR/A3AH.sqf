...
/* *********************************www.infiSTAR.de********************************* */
diag_log '<infiSTAR.de> VERSION: infiSTAR.de 03-03-2018 16-34-54 (v258)';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='swagg730@gmail.com';
INFISTARVERSION='03-03-2018 16-34-54 - v258';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    swagg730@gmail.com - 03-03-2018 16-34-54 - v258 - %2 - %3',time,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [swagg730@gmail.com - 03-03-2018 16-34-54 - v258 - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    swagg730@gmail.com - 03-03-2018 16-34-54 - v258 - %2 - %3',_t,serverName,productVersion];
...
FNC_A3_CUSTOMLOG = compileFinal "
	...
	diag_log format['<infiSTAR.de>%1| %2   [03-03-2018 16-34-54 - v258]',_logname,_logentry];
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['a','7','e','7','3','8','6','a','c','9','f','c','5','2','5','2','d','d','9','7','4','b','a','b','f','d','8','4','e','9','a','0','7'];
...
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + 'AdminReq tokenreceived is empty (v258)';
	_log call FNC_A3_SURVEILLANCELOG;
};
if(count _array < 2)exitWith
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + format['AdminReq bad array: %1 (v258)',_array];
	_log call FNC_A3_SURVEILLANCELOG;
};
if(_clientNetID isEqualTo '')exitWith
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + 'AdminReq clientNetID is empty (v258)';
	_log call FNC_A3_SURVEILLANCELOG;
};
...
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + 'AdminReq _ObjFromNetID is Null (v258)';
	_log call FNC_A3_SURVEILLANCELOG;
};
...
{
	_log = format['%1(%2) | AdminReq - Bad PUID / Token: Token received [%3] belongs to [%4] and not [%2] (v258)',_clientName,_clientUID,_tokenreceived,_uid_by_token];
	_log call FNC_A3_SURVEILLANCELOG;
};
...
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + format['AdminReq _playerObj != _ObjFromNetID - %1/2  (v258)',_playerObj,_ObjFromNetID];
	_log call FNC_A3_SURVEILLANCELOG;
...
_FNC_AH_KICKLOGSPAWN =
{
	...
	if(isNil '_input')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'FNC_AH_KICKLOGSPAWN _this is Nil (v258)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_tokenreceived')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + '_tokenreceived _this is Nil (v258)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _tokenreceived != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_tokenreceived wrong type %1 (v258)',typeName _tokenreceived];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_arraysent')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + '_arraysent _this is Nil (v258)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _arraysent != 'ARRAY')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_arraysent wrong type %1 (v258)',typeName _arraysent];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(typeName _netId != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_netId wrong type %1 (v258)',typeName _netId];
		_log call FNC_A3_SURVEILLANCELOG;
	};
...
fn_adminConnected_infiSTAR = {
	...
	if(isNil'infiSTAR_MAIN_CODE')exitWith
	{
		...
		diag_log '<infiSTAR.de> ERROR - infiSTAR_MAIN_CODE is NIL! (v258)';
	};
	...
	"; if(_LogAdminActions)then{ _A3AHstring = _A3AHstring + "
		_fnc_adminLog = if(_uid in "+str _devs+")then{{}}else{
			diag_log format['<infiSTAR.de> ******ADMIN-LOGIN******: %1(%2) (v258)',_name,_uid];
...
_log call FNC_A3_CONNECTLOG;
_m = missionNameSpace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=12367';missionNameSpace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
...
"; }; _A3AHstring = _A3AHstring + "
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '24734x2';
	_devLog = format['<infiSTAR.de> %1 VERSION: infiSTAR.de 03-03-2018 16-34-54 (v258) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_res = [VERSION_CHECK_URL_FULL+'&id=4af4c0b1cd6a&li=7e7386ac9fc5252dd974babfd84e9a07&pr=A3'] call FN_ARMA_FETCHDATA;
